...
final class ResourceDropdownTest extends TestCase
{
    ...
    #[Test]
    public function resourceDropdownAdapterCanPrependOptions(): void
    {
        $source = new ResourceDropdownAdapter(StatusEnum::dropdown())
            ->prepend(['value' => 'all', 'label' => 'Все'])
            ->prepend(['value' => 'deleted', 'label' => 'Удаленные']);

        $options = Resource::dropdown($source, false);

        $this->assertSame('deleted', $options[0]['value']);
        $this->assertSame('Удаленные', $options[0]['label']);
        $this->assertSame('all', $options[1]['value']);
        $this->assertSame('Все', $options[1]['label']);
        $this->assertSame('active', $options[2]['value']);
    }

    #[Test]
    public function resourceDropdownAdapterCanAppendOptions(): void
    {
        $source = new ResourceDropdownAdapter(StatusEnum::dropdown())
            ->append(['value' => 'archived', 'label' => 'Архив']);

        $options = Resource::dropdown($source, false);

        $last = $options[array_key_last($options)];
        $this->assertSame('archived', $last['value']);
        $this->assertSame('Архив', $last['label']);
    }

    #[Test]
    public function resourceDropdownKeepsOptionMeta(): void
    {
        $source = new ResourceDropdownAdapter([
            [
                'value' => 1,
                'label' => 'One',
                'meta'  => [
        ...
        $this->assertSame(1, $options[0]['value']);
        $this->assertSame('One', $options[0]['label']);
        $this->assertSame(['email' => 'one@example.com'], $options[0]['meta']);
    }
